...
import DetailsCheckout from "@/components/DetailCheckout";
...
export default function Home() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/users/checkout`, {
      ...
      .then((data) => {
        ...
        setTotalPrice(data.checkout.total_price);
  ...
  const onConfirmOrder = async () => {
    const response = await fetch("http://localhost:3000/api/users/checkout", {
      method: "PATCH",
      credentials: "include",
      headers: {
        "Content-Type": "application/json",
    ...
    <>
      ...
      <div className="px-16 flex flex-col gap-3 ">
        ...
        <div className="w-full bg-white rounded-md  flex flex-col pt-5 ">
          <DetailsCheckout groupItems={groupItems} />
          ...
          <div className="border-t w-full  h-16 flex justify-end pr-5 gap-2 items-center">
            <span>Tổng số tiền ({count} sản phẩm):</span>
            <span className="text-beamin font-bold">
              đ{totalPrice.toLocaleString("en-US")}
            </span>
          </div>
        </div>
