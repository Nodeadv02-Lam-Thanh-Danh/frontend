...
export default async function HeaderNav() {
  ...
  const profile: Profile = json.user;

  const res2 = await fetch(`http://localhost:3000/api/users/order?getCount=true`, {
    credentials: "include",
    headers: {
      "Content-Type": "application/json",
      Cookie: cookieStore.toString(),
  ...
  return (
    <div className="w-full h-fix bg-white flex flex-row fixed  py-3 gap-4 justify-items-center	justify-center z-50	">
      ...
      <div className="flex-none w-1/4  flex flex-row items-center  py-2">
        ...
        <Profile orderCount={count} lastName={profile ? profile.last_name : ""} />
        <Button
          href="/cart"
          type="text"
          style={{
